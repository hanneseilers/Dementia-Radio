...
String filesList[200];    // list of available file names
uint8_t nFilesLeft;       // count of files still to play
uint8_t nFilesPlayed;     // count of files already played
...
void onVolume() {
  ...
  long adcValue = analogRead(ADC_VOLUME);
  
  for( uint8_t i=0; i++; i < 50){
    adcValue = adcValue + analogRead(ADC_VOLUME);
    adcValue = adcValue / 2;
  }
    ...
    if( filenameLength > 5 ){
      ...
      if( nRandom == 0 ){
        ...
        if( !SD.exists(filename) ){
          ...
          nFilesLeft--;
          filesList[i] = "-";   // set filename that is not long enough
